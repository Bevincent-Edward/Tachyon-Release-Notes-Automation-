...
function App() {
  ...
  const FeaturePie = ({ data }) => {
    ...
    const points = data.categories.map((cat, i) => {
      const point = getPoint(data.angles[i], data.scores[i]);
      return `${point.x},${point.y}`;
    }).join(' ');
    
    return (
      <div className="radar-chart-viz">
        <svg viewBox="0 0 300 300" className="radar-svg">
          {/* Grid circles */}
          {[20, 40, 60, 80, 100].map(level => (
            <polygon
              key={level}
              points={data.categories.map((_, i) => {
                const point = getPoint(data.angles[i], level);
                return `${point.x},${point.y}`;
              }).join(' ')}
              fill="none"
              stroke="#e5e7eb"
          ...
          {data.categories.map((_, i) => {
            const point = getPoint(data.angles[i], maxScore);
            return (
              <line
                key={i}
                x1={centerX}
        ...
        <div className="radar-legend">
          {data.categories.map((cat, i) => (
            <div key={i} className="radar-legend-item">
              <span className="radar-dot" style={{ backgroundColor: '#667eea' }} />
              <span className="radar-label">{cat}</span>
              <span className="radar-value">{data.scores[i]}%</span>
            </div>
          ))}
        </div>
      </div>
