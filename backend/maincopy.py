...
# Groq imports (PRIMARY - Fastest)
from openai import AsyncOpenAI

# Gemini imports (FALLBACK - When Groq limit reached)
...
async def enhance_text_with_llm(feature: RawFeature) -> Optional[dict]:
    """
    FALLBACK CHAIN: Groq → Gemini → None (regex fallback)
    1. Try Groq first (fastest, LPU-accelerated)
    2. If Groq fails (rate limit), try Gemini (token-efficient)
    3. If both fail, return None for regex fallback
    
    Uses JSON mode for structured output. ASYNC for parallel processing.
    ...
    if groq_client and groq_model_name:
        try:
            ...
            print(f"⚠️  [GROQ] Failed for {feature.feature_name}: {type(e).__name__}: {e}")
    ...
    if gemini_model:
        try:
            ...
            # Gemini doesn't support response_format, so we prompt for JSON
            ...
            print(f"⚠️  [GEMINI] Failed for {feature.feature_name}: {type(e).__name__}: {e}")
            print(f"   → Falling back to regex processing...")
    
    # ============ BOTH FAILED ============
    print(f"❌ Both Groq and Gemini failed for {feature.feature_name}")
    return None


def get_system_prompt() -> str:
    """Shared system prompt for both Groq and Gemini"""
    return """You are an expert technical writer rewriting engineering release notes for customers.

RULES:
1. Translate jargon ("God Tenant", "UnZipp", "Day -1", "Blocked on bank") to professional language
2. Use PRESENT tense only. Never: "Previously", "Currently", "Now", "Existing"
3. Active voice only
4. Titles: Start with noun, Title Case, max 12 words
5. Description: Past tense, starts with "Introduced..."
6. Multiple items → use enhancement_bullets or impact_bullets arrays (no periods)
7. Bold ONLY technical acronyms: **UPI**, **SFTP**, **API**, **NPCI**, **ANV**, **POS**
8. DO NOT bold brands: Visa, Mastercard, Tachyon (capitalize only)
9. Remove internal refs: "Feedback |", "Pushpa", "Ruby", "Saturn", "Zeta" → use "the system"
10. No leading commas, capitalize sentence starts"""
...
async def process_release_notes(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Groq LPU can handle high concurrency!
        print(f"🚀 Processing {len(publishable)} features in parallel with Groq LPU (max 5 concurrent)...")

        # Semaphore to limit concurrent API calls (Groq can handle 5+ concurrent easily)
        semaphore = asyncio.Semaphore(5)  # High concurrency for Groq's fast LPU
